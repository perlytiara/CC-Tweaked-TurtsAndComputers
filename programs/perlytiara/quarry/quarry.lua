...
elseif (#tArgs >= 3) then
	...
	if (#tArgs > 4) then
		bStartBelow = (tonumber(tArgs[5])==1)
	else
		bStartBelow = false
	end
else
...
local myTurnLeft = turnLeft
local myTurnRight = turnRight
if xSign == -1 then
...
if bStartBelow then
	if not goDown() then
		print("Cannot dig below starting position. Aborting.")
		return
	end
	tY = -1
	lowestY = -sizeY
...
while true do -- This loops digging layers
  ...
  local thisTraversal = traversal
  if zSign == -1 then thisTraversal = traversal + 1 end
  ...
  for column=1,sizeX  do  -- This loops sizeX times digging out columns
    ...
    if (column<sizeX) then
      ...
      local evenLayer = ((thisTraversal%2)==0)
